...
const Prev_games = [];
const Prev_games_darts = [];
const Prev_games_esports = [];
...
function isDifferenceTenPercentOrGreater(a, b) {
    ...
    return parseFloat(difference) >= 0.09;
  }
...
async function check_changes(prev, current) {
    ...
    current.forEach((currentItem) => {
        ...
        if (!prevItem) {
            ...
            // console.log(' currentItem ', currentItem);
                if(currentItem.sport === 'Darts' || currentItem.sport === 'E Sports'){
                ...
                const currentSpread = currentItem?.spreads?.find((currentSpread) => prevSpread?.team1Spread == currentSpread?.team1Spread && prevSpread?.team2Spread == currentSpread?.team2Spread)
                        ...
                        if (isDifferenceTenPercentOrGreater(prevSpread?.team1SpreadOdd, currentSpread?.team1SpreadOdd)) {
                            ...
                            let try_me = `${prevItem?.sport};${prevItem?.league};${prevItem?.team1} vs ${prevItem?.team2};${odds_to_beat};${ROI5};${ROI10};${prevItem?.team1};${prevSpread?.team1Spread};${prevSpread?.team1SpreadOdd} -> ${currentSpread?.team1SpreadOdd};${difference} \n`
                        ...
                        if (isDifferenceTenPercentOrGreater(prevSpread?.team2SpreadOdd, currentSpread?.team2SpreadOdd)) {
                            ...
                            let try_me = `${prevItem?.sport};${prevItem?.league};${prevItem?.team1} vs ${prevItem?.team2};${odds_to_beat};${ROI5};${ROI10};${prevItem?.team2};${prevSpread?.team2Spread};${prevSpread?.team2SpreadOdd} -> ${currentSpread?.team2SpreadOdd};${difference} \n`
                ...
                if(sent_not.includes(prevItem?.team1+'total')){}
                    ...
                    const currentTotal = currentItem?.totals?.find((currentTotal) => prevTotal?.gameTotal == currentTotal?.gameTotal)
                        ...
                        if (isDifferenceTenPercentOrGreater(prevTotal?.over, currentTotal?.over)) {
                            ...
                            const ROI5= ((0.04 + (1*under_perc))/over_perc)+1

                            let try_me = `${prevItem?.sport};${prevItem?.league};${prevItem?.team1} vs ${prevItem?.team2};${odds_to_beat};${ROI5};${ROI10};Over;${prevTotal?.gameTotal};${prevTotal?.over} -> ${currentTotal?.over};${difference} \n`
                        ...
                        if (isDifferenceTenPercentOrGreater(prevTotal?.under, currentTotal?.under)) {
                            ...
                            let try_me = `${prevItem?.sport};${prevItem?.league};${prevItem?.team1} vs ${prevItem?.team2};${odds_to_beat};${ROI5};${ROI10};Under;${prevTotal?.gameTotal};${prevTotal?.under} -> ${currentTotal?.under};${difference} \n`
                ...
                if(sent_not.includes(prevItem?.team1+'ML')){}
                        ...
                        if (isDifferenceTenPercentOrGreater(prevItem?.team1_ML, currentItem?.team1_ML)) {
                            ...
                            let try_me = `${prevItem?.sport};${prevItem?.league};${prevItem?.team1} vs ${prevItem?.team2};${odds_to_beat};${ROI5};${ROI10};${prevItem?.team1};- Vencedor;${prevItem?.team1_ML} -> ${currentItem?.team1_ML};${difference} \n`
                        ...
                        if (isDifferenceTenPercentOrGreater(prevItem?.team2_ML, currentItem?.team2_ML)) {
                            ...
                            let try_me = `${prevItem?.sport};${prevItem?.league};${prevItem?.team1} vs ${prevItem?.team2};${odds_to_beat};${ROI5};${ROI10};${prevItem?.team2};- Vencedor;${prevItem?.team2_ML} -> ${currentItem?.team2_ML};${difference} \n`
                ...
                if (message.length === 0){
                    ...
                    const sortedData = message.sort((a, b) => {
                        // Split each string by semicolon
                        const aValue = parseFloat(a.split(';').pop());
                        const bValue = parseFloat(b.split(';').pop());
        ...
        Alternativas:`;
                ...
                // console.log(message)
                const sortedData = message_totals.sort((a, b) => {
                    // Split each string by semicolon
                    const aValue = parseFloat(a.split(';').pop());
                    const bValue = parseFloat(b.split(';').pop());
    ...
    for (const leagueData of leagues) {
        ...
        for (const gameData of gamesData) {
            ...
            if(team1.includes('(Points)')){
                ...
                const spreadData = gameData[8][1][0];
                try{
                    var team1_ML = gameData[8][1][2][1];
                }catch{
                    var team1_ML = 0
                }
                try{
                    var team2_ML = gameData[8][1][2][0];
                }catch{
                    var team2_ML = 0
                }
                try{
                    var draw = gameData[8][1][2][2];
                }catch{
                    var draw = 0
                }
                ...
                const totalData = gameData[8][1][1];
                ...
                if (trigger === 0){
                    Prev_games.push({sport, league, team1, team2, spreads, totals, team1_ML, team2_ML, draw});
                }else{
                    current_games.push({sport, league, team1, team2, spreads, totals, team1_ML, team2_ML, draw });
...
(async () => {
    ...
    // { name: 'e_sports', sp: 12, processFunction: processJsonData_esports },
    const sports = [
